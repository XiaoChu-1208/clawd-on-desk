...
### 채팅 패널 (Coach 모드)
- **플로팅 대화 창** — 다중 버블 스택 채팅 창이 펫 위에 표시됩니다. 내 메시지는 사용자 버블로, Clawd 응답은 코치 버블로 구분 표시됩니다.
- **이미지 붙여넣기** — 입력창에 클립보드 이미지를 붙여넣으면 전송 전 미리보기 칩이 표시되고, 전송 후에는 버블 안에 썸네일로 렌더링됩니다.
- **슬래시 명령어** — `/say`, `/…` 등의 명령어를 입력하면 검은색 터미널 스타일 버블로 렌더링되어 일반 대화와 구분됩니다.
- **버블 텍스트 복사** — 버블을 클릭하면 전체 텍스트가 클립보드에 복사됩니다(짧은 "복사됨" 확인 표시). 드래그 선택 후 Cmd+C로 일부만 복사할 수도 있습니다.
- **설정 → Voice(음성 설정)** — 마이크, 말하기 볼륨, 음성 효과, 말하기 인터럽트 감도, 웨이크 워드("Claude"), 노크 웨이크, 모델 등급(Haiku / Sonnet / Opus)을 한 탭에서 설정합니다.
- **설정 → Chat(대화 설정)** — 모델 선택과 세션 관리(목록, 이름 변경, 전환, 신규 생성, 삭제)가 별도 탭으로 분리되어 Voice 탭은 오디오 컨트롤에만 집중합니다.

### 음악 반응형 펫
- **음악에 맞춰 춤** — 설정된 음악 앱이 실행 중이고 시스템에서 실제로 오디오가 출력 중이면 펫이 자동으로 그루브 애니메이션으로 전환됩니다. 일시 정지하거나 멈추면 대기 상태로 돌아옵니다.
- **음악 앱 자유 선택** — `Settings…` → `Voice` → 음악 앱에서 선택: 끄기 / 网易云音乐(넷이즈) / Apple Music / QQ Music / 汽水音乐 / Spotify, 또는 커스텀 프로세스 이름 입력
- **제로 커플링 설계** — `music-watch.js`는 독립적인 선택 사항 헬퍼 스크립트로, 시스템 오디오 출력을 감지해 Clawd의 `/say` HTTP 엔드포인트로 상태를 전달합니다. 핵심 애니메이션 파이프라인은 영향받지 않습니다.
