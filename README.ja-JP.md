...
### チャットパネル（Coach モード）
- **フローティングチャットウィンドウ** — 複数バブルのスタック式チャットウィンドウがペットの上に表示されます。自分のメッセージはユーザーバブル、Clawd の返答はコーチバブルとして区別されます。
- **画像の貼り付け** — 入力欄にクリップボードの画像を貼り付けると、送信前にプレビューチップが表示され、送信後はバブル内にサムネイルとしてレンダリングされます。
- **スラッシュコマンド** — `/say`、`/…` などのコマンドを入力すると、黒いターミナル風バブルとしてレンダリングされ、通常の会話と視覚的に区別されます。
- **バブルのテキストをコピー** — バブルをクリックするとテキスト全体がクリップボードにコピーされます（短い「コピーしました」確認表示あり）。ドラッグ選択後 Cmd+C で部分コピーも可能です。
- **設定 → Voice（音声設定）** — マイク、話す音量、音声エフェクト、割り込み感度、ウェイクワード（「Claude」）、ノックウェイク、モデル（Haiku / Sonnet / Opus）をひとつのタブで管理できます。
- **設定 → Chat（チャット設定）** — モデル選択とセッション管理（リスト・名前変更・切替・新規作成・削除）が独立したタブに分かれており、Voice タブは音声コントロールに集中できます。

### 音楽反応型ペット
- **音楽に合わせてグルーヴ** — 設定した音楽アプリが起動中でシステムから実際にオーディオが出力されると、ペットは自動でグルーヴアニメーションに切り替わります。一時停止・停止するとアイドル状態に戻ります。
- **音楽アプリを自由に選択** — `Settings…` → `Voice` → 音楽アプリから選択：オフ / 網易云音乐（NetEase）/ Apple Music / QQ Music / 汽水音乐 / Spotify、またはカスタムプロセス名を入力
- **ゼロカップリング設計** — `music-watch.js` は独立したオプションのヘルパースクリプトで、システムのオーディオ出力を確認して Clawd の `/say` HTTP エンドポイントに状態を送信します。コアのアニメーションパイプラインには影響しません。
