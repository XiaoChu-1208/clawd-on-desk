...
### 對話面板（Coach 模式）
- **浮動對話視窗** — 多氣泡堆疊式聊天視窗浮在桌寵上方，你的訊息顯示為使用者氣泡，Clawd 的回覆顯示為 coach 氣泡
- **貼上圖片** — 在輸入框貼上剪貼簿圖片，送出前顯示預覽小片，圖片送出後在氣泡內渲染為縮圖
- **斜線命令** — 輸入 `/say`、`/…` 等命令時，渲染為黑色終端機風格氣泡，與一般對話氣泡明確區分
- **點擊氣泡複製** — 點擊任意氣泡即可複製全文到剪貼簿（短暫顯示「已複製」提示）；也可以拖選後 Cmd+C 複製片段
- **設定 → Voice（語音設定）** — 單一標籤頁統一管理麥克風、說話音量、語音效果、說話打斷靈敏度、喚醒詞（「Claude」）、敲擊喚醒，以及模型等級（Haiku / Sonnet / Opus）
- **設定 → Chat（對話設定）** — 模型選擇和工作階段管理（清單、重新命名、切換、新建、刪除）獨立成標籤頁，Voice 標籤專注於音訊控制

### 音樂響應桌寵
- **隨音樂律動** — 當你設定的音樂 App 正在執行且系統確實有音訊輸出時，桌寵自動進入律動動畫；暫停或停止後回到待機狀態
- **自由選擇音樂 App** — 在 `設定…` → `Voice` → 音樂應用選擇：關閉 / 網易雲音樂 / Apple Music / QQ 音樂 / 汽水音樂 / Spotify，或輸入自訂行程名稱
- **零耦合設計** — `music-watch.js` 是獨立的可選輔助腳本，探測系統音訊輸出後向 Clawd 的 `/say` HTTP 接口推送狀態，核心動畫管線不受影響
